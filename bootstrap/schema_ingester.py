...
def get_class_id(create_class_url, create_schema_url, headers, class_title, data, reset_demo):
    """
    ...
    try:
        ...
        if(reset_demo == "True" and "status" in error_json and error_json["status"] == 400) :
            id_param = get_id_from_errordetail(error_json)
            existing_class_id = create_class_url + "/" + id_param
            print ("Deleting class with id1 ", existing_class_id)
            #delete_class_id(existing_class_id, create_schema_url, headers)
            
            return get_class_id(create_class_url, create_schema_url, headers, class_title, data, reset_demo)
...
def get_mixin_id(create_mixin_url, headers, mixin_title, data, class_id, tenant_id, mixin_definition_title, reset_demo):
    ...
    try:
        ...
        http_error_json_msg = u'%s' % (http_err)
        print ("http_error_json_msg", http_error_json_msg)
        error_json = json.loads(http_error_json_msg)
        if(reset_demo == "True" and "status" in error_json and error_json["status"] == 400) :
            id_param = get_id_from_errordetail(error_json)
            if(id_param != "" ):
